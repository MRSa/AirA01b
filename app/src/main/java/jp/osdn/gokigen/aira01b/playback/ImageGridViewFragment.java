...
public class ImageGridViewFragment extends Fragment  implements AdapterView.OnItemSelectedListener, CompoundButton.OnCheckedChangeListener, View.OnClickListener, MultiFileDownloadConfirmationDialog.Callback, ImageDownloader.Callback, MultiFileDeleteConfirmationDialog.DeleteConfirmationCallback
{
    ...
    public void confirmDeleteSelection(int selectedButtonId, final boolean withRaw, final boolean onlyRaw)
    {
        try
        {
            Log.v(TAG, " confirmDeleteSelection (with RAW : " + withRaw + " only RAW : " + onlyRaw + ")");
            final Activity activity = getActivity();
            if (activity == null)
            {
                Log.v(TAG, " CAN NOT GET ACTIVITY... ABORT DELETE");
                return;
            }
    ...
    public void confirmSelection(int selectedButtonId, boolean withRaw)
    {
        ...
        float downloadSize = OLYCamera.IMAGE_RESIZE_NONE;
        if (selectedButtonId == R.id.radio_download__1024x768)
        {
            downloadSize = OLYCamera.IMAGE_RESIZE_1024;
        }
        else if (selectedButtonId == R.id.radio_download__1600x1200)
        {
            downloadSize = OLYCamera.IMAGE_RESIZE_1600;
        }
        else if (selectedButtonId == R.id.radio_download__1920x1440)
        {
            downloadSize = OLYCamera.IMAGE_RESIZE_1920;
        }
        else if (selectedButtonId == R.id.radio_download__2048x1536)
        {
